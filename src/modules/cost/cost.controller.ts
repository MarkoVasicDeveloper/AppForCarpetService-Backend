...
import { AddCostDto } from 'src/modules/cost/DTO/add.cost.dto';
import { EditCostDto } from 'src/modules/cost/DTO/edit.cost.dto';
...
import { CostService } from 'src/modules/cost/cost.service';
...
export class CostController {
  ...
  @UseGuards(RolleCheckerGard)
  @SetMetadata('allow_to_roles', ['user', 'administrator'])
  async getAllCosts(@Param('id') userId: number, @Param('costid') costId: number): Promise<Cost[]> {
  ...
  async getAllCostsBySupplier(
    ...
    return await this.costService.getAllCostsBySupplier(costId, userId, supplierId);
  }
}
