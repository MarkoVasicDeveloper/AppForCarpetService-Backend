...
import { AddCostsDto } from 'src/modules/costs/DTO/add.costs.dto';
import { EditCostsDto } from 'src/modules/costs/DTO/edit.costs.dto';
...
export class CostsService {
  constructor(@InjectRepository(Costs) private readonly costsService: Repository<Costs>) {}

  async addCosts(data: AddCostsDto, userId: number): Promise<Costs | ApiResponse> {
  ...
  async editCosts(data: EditCostsDto, userId: number): Promise<Costs | ApiResponse> {
    const costs = await this.costsService.findOne({
      where: {
        title: data.title,
        userId: userId,
